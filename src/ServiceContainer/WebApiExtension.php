...
{
    /**
     * User PHPMatcher traits for PHPUnit
     */
    ...
    /**
     * Checks that response body matches the given JSON from PyString.
     * For docs on matching see: coduo/php-matcher
     * @param PyStringNode $jsonString
     *
    ...
    {
        $response = $this->response->getBody()->__toString();
        $expected = $jsonString->__toString();

        $this->assertMatchesPattern($expected, $response);
    }

    /**
     * Checks that the response is valid JSON and headers are set correctly.
     *
     * @Then the response is valid json
     */
    public function theResponseIsValidJson()
    {
        $headers = $this->response->getHeaders();

        Assertions::assertArrayHasKey('Content-Type', $headers);
        Assertions::assertContains('application/json', $headers['Content-Type']);

        Assertions::assertJson($this->response->getBody()->__toString());
    }
